...
def main():
    ...
    def rewardMaster(state, action, nextState): return [-reward for reward in rewardWolf(state, action, nextState)]

    def rewardFunc(state, action, nextState): return \
    ...
    def changeJointDampingProperty(envDict, geomPropertyDict): return changeJointProperty(envDict, geomPropertyDict, '@damping')
    ...
    def changeJointFrictionlossProperty(envDict, geomPropertyDict): return changeJointProperty(envDict, geomPropertyDict, '@frictionloss')
    ...
    def isTerminal(state): return False
    ...
    def observeOneAgent(agentID): return Observe(agentID, wolvesID, sheepsID, masterID, getPosFromAgentState, getVelFromAgentState)
