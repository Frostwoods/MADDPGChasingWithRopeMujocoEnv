...
from env.multiAgentEnv import GetActionCost, RewardSheep, RewardWolf, Observe, IsCollision, getPosFromAgentState, \
    getVelFromAgentState, PunishForOutOfBound, StayInBoundaryByReflectVelocity, ResetMultiAgentNewtonChasing, \
    TransitMultiAgentChasingForExp, ReshapeAction, GetCollisionForce, ApplyActionForce, ApplyEnvironForce, \
    IntegrateState, getPosFromAgentState, getVelFromAgentState, ResetMultiAgentChasingWithVariousSheep
...
def main():
    ...
    print(
        ...
        format(numWolves, numSheeps, numBlocks, maxEpisode, maxTimeStep, sheepSpeedMultiplier, individualRewardWolf,
               str(saveAllmodels)))

    dataFolder = os.path.join(dirName, '..', '..', 'data')
    mainModelFolder = os.path.join(dataFolder, 'model')
    modelFolder = os.path.join(mainModelFolder, 'originEnv0618', 'indvidulReward={}'.format(individualRewardWolf),
                               '{} wolves, {} sheep, {} blocks'.format(numWolves, numSheeps, numBlocks))

    if not os.path.exists(modelFolder):
        os.makedirs(modelFolder)

    # -------agent size & speed setting------
    ...
    rewardSheep = RewardSheep(wolvesID, sheepsID, entitiesSizeList, getPosFromAgentState, isCollision,
                              punishForOutOfBound,
                              collisionPunishment=collisionReward)  # TODO collisionReward = collisionPunishment
    ...
    trainMADDPGModels = TrainMADDPGModelsWithBuffer(updateParameters, trainActor, trainCritic, sampleBatchFromMemory,
    ...
    saveModels = [SaveModel(modelSaveRate, saveVariables, getTrainedModel, modelPath + str(i), saveAllmodels) for
                  i, getTrainedModel in enumerate(getModelList)]
